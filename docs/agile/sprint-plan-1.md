<!-- Template เต็มไฟล์สำหรับสร้าง docs/agile/sprint-plan-[NN].md ของ Sprint ไหนก็ได้ -->

<!-- ดึง Story ของ Sprint นี้มาจาก docs/agile/02-sprint-backlog.md -->

<!-- Sprint 1: เปลี่ยนชื่อ sprint-01.md จาก Lab 07 เป็น sprint-plan-01.md แล้วแทนที่เนื้อหาด้วย template นี้ -->

<!-- Sprint 2-4 ในแลปถัดไป: คัดลอกไฟล์นี้ทั้งไฟล์ไปสร้าง sprint-plan-02.md, sprint-plan-03.md, sprint-plan-04.md ตามลำดับ -->

# Sprint [1] Plan

**Sprint Goal:** [เป้าหมายหลักของ Sprint นี้ในหนึ่งประโยค]
**ระยะเวลา:** [วันที่เริ่ม] — [วันที่สิ้นสุด]
**Team:** [รายชื่อ]

---

## Sprint Backlog

| # | User Story                                                                                    | รับผิดชอบ | MoSCoW      | Estimate (SP) | Status         |
| - | --------------------------------------------------------------------------------------------- | ------------------ | ----------- | ------------- | -------------- |
| 0 | [User Story ของ Sprint นี้ จาก 02-sprint-backlog.md]                                 | [ชื่อ]         | Must Have   | [SP]          | 🔲 Todo        |
| 1 | As i want to Make Hover Card read Descrition and Click to Use Card                            | Aris               | Must Have   | 5             | ✅ Done        |
| 2 | As a game maker, I want Maze system, So that can fit to gerne Roglike                         | Aris               | Must Have   | 5             | ✅ Done        |
| 3 | As a game maker, I want Random encounter system, So that can fit to gerne Roguelike           | Aris               | Must Have   | 5             | 🔄 In Progress |
| 4 | As an artist, I want enemy animation, so that it have movement                                | Ohm                | Must Have   | 5             | 🔄 In Progress |
| 5 | As a player, I want to see my remaining Hp/Sanity, so that I know how close I am to game over | Aris/Ohm           | Should have | 5             | 🔄 In Progress |
| 6 | ❌                                                                                            | ❌                 | ❌          | ❌            | ❌             |
| 7 | ❌                                                                                            | ❌                 | ❌          | ❌            | ❌             |
| 8 | ❌                                                                                            | ❌                 | ❌          | ❌            | ❌             |
| 9 | ❌                                                                                            | ❌                 | ❌          | ❌            | ❌             |

## Status Legend

- 🔲 Todo
- 🔄 In Progress
- ✅ Done
- ❌ Blocked

---

## Tasks

### Story 1 — [ชื่อย่อของ Story]

- [ ] [Task ย่อย]  [owner:: ชื่อ]  [estimate:: Nh]  [status:: todo]
- [ ] [Task ย่อย]  [owner:: ชื่อ]  [estimate:: Nh]  [status:: todo]

---

## Daily Notes

### [วันที่]

**เมื่อวาน:** ...
**วันนี้:** ...
**Blocked:** ...

---

## Links

- [[docs/gdd/00-concept|GDD Concept]]
- [[docs/agile/01-product-backlog|Product Backlog]]
- [[docs/agile/02-sprint-backlog|Sprint Backlog]]
